...
It also keeps a project's research state in `.rb/`: the questions, hypotheses, experiments with every setting and where its value came from, claims with their criteria, the evidence attached to them, and the decisions people made. If the project has a `.rb/` directory, run `rb context` before anything else and read the section "Research state" below: it is the state of the work, and it replaces anyone's summary of it.
...
`0` ok · `1` a saved check failed or is missing, or a case is flagged (`rb check run`, default `--fail-on any`; `rb run` only with `--fail-on`), a research-state gate failed (`rb status --fail-on`), or a setting did not verify (`rb spec verify`, `rb spec set --verify`); the JSON has `ok: true` because the result is data · `2` invalid input or config, including a command line that does not parse (`E_USAGE`) · `3` environment failure (`rb doctor` when something fails; adapter, hook, device or adapter-agreement problems). Read `errors[].fix`.
...
| `E_OBJECT_NOT_FOUND` | no object with that id | `rb status --json` lists every object; `rb show <experiment>` its settings, claims and evidence |
...
| `E_FROZEN` | the experiment is frozen and this changes a setting or adds a claim | a person passes `--amend "<reason>"`; the amendment is kept |
...
| `E_SOURCE_UNRESOLVED` | the source was read and does not state the value, or could not be read | the setting stays provisional; fix the path, line, quote or commit, or set the value the source states |
...
| `E_LEDGER_CORRUPT` | a file under `.rb/` is not valid, usually a hand edit or a merge conflict in `log.jsonl` | `git diff .rb/`, restore it, and change state with `rb` commands |
...
**The rule: you and your agents propose; `rb` verifies; people decide.**

- You may add questions, hypotheses, assumptions, experiments, settings with their sources, claims, and evidence.
- A setting you give is `provisional`: it stands for now and has not been checked. Only `rb spec verify` makes it `verified`, by reading the source you gave and finding the value stated there. It records the commit, the file's hash and the text it read, and re-checks them on every read: if the file changes, or the value no longer matches what was read, `rb status` reports the setting as stale. Never edit `.rb/` by hand: a setting marked verified without what `rb` read fails to load (`E_LEDGER_CORRUPT`).
- `verified` means the source you named states the value. It does not mean a run used that value: a config line is evidence of what the config says.
- A claim's verdict is computed from the evidence every time it is read, and never written. A claim is **established** only when its evidence meets the criterion, no required setting is unknown, no verified source has changed, a frozen spec still matches its freeze record, and the evidence is not only synthetic. Report "supported, not established" exactly as that.
...
rb spec set int8 lr 1e-4 --source configs/train.yaml:17 --verify       # provisional, then verified if line 17 states 1e-4
rb spec set int8 seed --unknown                                        # nobody has found it: blocks the claims until found (--optional if it should not)
rb spec set int8 cuda 12.4 --source https://example.com/env            # recorded, stays provisional: a url cannot be checked by reading a file
rb spec verify int8                                                    # re-read every source; a source that no longer states the value demotes the setting
rb claim add "INT8 keeps quality" --experiment int8 --metric delta_epe --at-most 0.05 --hypothesis h1
rb freeze int8                                                         # the human, before held-out results: later changes to settings or new claims need --amend "<reason>"
rb evidence attach int8 --metric delta_epe=0.0486 --command "python eval.py --int8"   # numbers from any evaluator
rb evidence attach int8 --run <run_id>                                 # or an rb run: candidate.<metric>, baseline.<metric>, change.<metric>, with_gt, regressions, flagged, ...
rb status --fail-on unestablished                                      # the CI gate: exit 1 unless every claim is established
rb show c1                                                             # the claim, its verdict, every observation and everything it rests on
rb context                                                             # the handoff pack a fresh session reads first
```

**Settings.** A value is a finite number, `true`/`false`, non-empty text, or a JSON list of those (`1e-4` is a number; `'"6"'` is the text 6). `--source` is `path:LINE`, `path` with `--quote "exact text"`, `run:PATH#/json/pointer` (a run directory means its `record.json`), `https://...`, or `note:text`; `--commit <sha>` reads the file at that commit in the file's own repository, which never goes stale; `--locator "§4.2"` says where a reader finds it. Paths are relative to where you run the command and are stored relative to the directory holding `.rb/`. What "states" means is strict, on purpose: a number is a standalone token compared exactly (an integer only by the same integer, a float by the same double, so `lr: 0.0001` states `1e-4` but `resnet50` does not state 50 and `1700000001` does not state 1700000000); text is a whole token (`adam` is not stated by `adamw`); a list by a bracketed or comma-separated group with the same elements in the same order. Changing a setting's value or source puts it back to `provisional`. `rb spec verify` exits 1 when any setting it tried did not verify, with `E_SOURCE_UNRESOLVED` (read, and the value is not there) or `E_SOURCE_UNVERIFIABLE` (a url, a note, or a file with no line or quote) per setting in `data.results`.

**Claims.** Exactly one criterion: `--at-most X`, `--at-least X`, or `--within TARGET --tolerance T`, fixed when the claim is written. `--metric` names a number the evidence reports. A claim with `--source` is a number someone else stated (a paper's table: save the text as a file and give `--quote` and `--locator`); a file source must state the target or the claim is refused, a url or note is recorded as unchecked, and the verdict is `reproduced` or `diverged`. Your own claims are `supported` or `refuted`; mixed evidence is `contested`; no evidence is `not_tested`. An observation is **borderline** when moving the criterion by a tenth of itself would change the call, the same rule as a borderline case in a review. It is **post hoc** when the claim was written after the evidence was attached, so its criterion may have been chosen after seeing the number. Say both when you report.

**Evidence.** `--metric name=value` (repeatable, each name once), `--from metrics.json` (a JSON object; nested keys join with dots; anything that is not a number, an empty object included, is named in a warning), or `--run` for an rb run or results file, found from anywhere in the project. A run with no ground truth attaches no error numbers, because an error that was not measured is not a zero. `--file` hashes an output into the evidence's `files`, `--link wandb=URL` records where else it lives, `--command` records what produced the numbers (rb did not run it). The receipt records the actor, the time, and the repository's state **when the evidence is attached**: the commit and whether the tree had uncommitted changes, with a hash covering the diff and the untracked files (`.rb/` itself left out); for an rb run, its own receipt and the hash of its `record.json`. Attach evidence from the tree that produced it. Evidence from `rb example` or the synthetic adapter is marked synthetic, and a claim resting only on it is never established. Wrong evidence is retracted with a reason by a person, never deleted: it stays on record and stops counting.

**Freezing.** `rb freeze <experiment>` records the hash of its spec: baseline, candidate, every setting's value and whether it is required, and every claim's criterion. After that, changing a setting (`rb spec set`) or adding a claim (`rb claim add`) fails with `E_FROZEN` unless a person gives `--amend "<reason>"`, which keeps the change, the reason and the hash before and after; baseline, candidate and criteria are fixed when created. Verifying a setting does not change the spec. A spec that no longer matches its freeze record, whoever changed it, is reported and keeps its claims from being established. Each observation says whether its evidence was attached before the freeze, under the current setup, or under a setup that has changed (or been amended) since.
...
**Reading the state.** `rb status` lists the claims with their verdicts, the experiments with their setting counts, and **Open**: open questions, hypotheses nothing tests, unchecked assumptions, unknown settings (required and optional), provisional settings and how to verify them, stale sources, frozen specs changed without an amendment, untested claims, supported claims that are not established and why, and refuted or contested claims nobody has decided on. Work from the open list. `--fail-on` takes `unestablished`, `unknown`, `untested`, `refuted`, `stale` (repeatable or comma-separated) and exits 1 when any applies. `rb status --json` lists every object, evidence included; `rb context` prints the same state as a handoff pack in Markdown. `rb schema claim` (and `investigation`, `question`, `hypothesis`, `assumption`, `experiment`, `evidence`, `decision`, `verdict`) prints each object's shape. Planned and not in this version: `rb mcp` (the same operations for agents over MCP, with the actor set on every call), `rb reproduce` (a paper's claims and settings as a pre-filled experiment), `rb publish` and `rb clone`.

When you report to a human from this state: quote the verdict and whether it is established; name every blocking unknown and every condition `rb show` lists; say which settings are provisional rather than verified; and never describe a claim as settled that `rb status` does not show as established.
...
For research state: `rb status` and `rb show <id>` are computed from `.rb/`, not from what anyone wrote about it. `.rb/log.jsonl` lists every write with its actor, and `git log -p .rb/` shows every change. A setting is verified only with the line or value `rb` read, which `rb show <experiment>` prints next to it.
