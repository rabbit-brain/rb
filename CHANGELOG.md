...
- Evidence rendering: `rb run` writes `rb-runs/<run>/evidence/<case>/` for the top flagged cases (`[evidence] level`, `top`; `--evidence none|standard|full`) and `rb case <run> <id> --render` for any case: inputs, current | candidate | ground-truth flow on one colour scale, per-pixel error maps, per-iteration filmstrips for both models, the trajectory plot, and `case.png` with the finding as caption. The re-run's numbers are checked against the run's and the result is written in the caption and the JSON. Needs numpy and pillow (`E_EVIDENCE_DEPS`); adapters opt in with `read_images` / `read_gt` (the raft adapter does).
