...
ERRORS: dict[str, tuple[str, str]] = {
    ...
    "E_NOT_AVAILABLE": ("This command is not part of this version of rabbit-brain.", "`rb rerun`, `rb open`, `rb mcp`, `rb reproduce`, `rb publish` and `rb clone` are planned. Use `rb run` (or `rb import` for results you already have) and `rb findings`; for research state, `rb status`."),
    ...
    "E_NO_INVESTIGATION": ("No research state here: no .rb/ in this directory or any directory above it.", "Run `rb investigation init \"<title>\"` in the project root (it creates .rb/), or cd into a project that has one."),
    ...
    "E_OBJECT_NOT_FOUND": ("No object with that id in this investigation.", "`rb status --json` lists every question, hypothesis, assumption, experiment, claim, evidence and decision; `rb show <experiment>` lists its settings, claims and evidence."),
    "E_OBJECT_INVALID": ("The values given do not make a valid object.", "Fix the field named in the message; `rb schema <kind>` prints the shape (claim, experiment, evidence, ...)."),
    "E_FROZEN": ("The experiment is frozen, and this would change its spec: a setting's value or whether it is required, or a new claim on it.", "A person passes --amend \"<reason>\" to change it anyway; the change, the reason and the spec hash before and after are kept."),
    ...
    "E_HUMAN_ONLY": ("Freezing, deciding, amending and retracting are a person's calls, and the actor (RB_ACTOR) is not a person.", "An agent proposes; hand this step to the human. If you are the human, unset RB_ACTOR or set RB_ACTOR=human:<name>."),
    "E_ACTOR_INVALID": ("RB_ACTOR is not in the form human:<name> or agent:<name>.", "Set RB_ACTOR=agent:claude-code (for example) in an agent's environment, or unset it."),
    "E_SOURCE_UNRESOLVED": ("The source was read and does not state the value, or could not be read.", "The setting stays provisional. Check the path, line, quote or commit, or set the value the source actually states: `rb spec set <exp> <name> <value> --source <file:line>`."),
    "E_SOURCE_UNVERIFIABLE": ("This source cannot be checked by reading a file: a url, a note, or a file source without a line or a quote.", "Save the page as a file and point at it with --quote, or point at a config file line or a run's record.json that states the value."),
    "E_LEDGER_CORRUPT": ("A file under .rb/ is not valid.", "It was probably edited by hand. `git diff .rb/` shows what changed; restore it, and change state with rb commands instead."),
