"""`rb review onboard`: turn a described setup into a working project, or into the honest shape of one.
...
def adapter_scaffold(brief: BriefV1, metric: Metric) -> str:
    """A file that imports, that `rb review doctor` can load, and whose unwritten parts each say what they must return.
...
Generated by `rb review onboard` from {BRIEF_NAME}. Five TODOs; nothing else needs changing.
Run `rb review doctor` after each one and it will tell you which step you are on.
...
class {cls}:
    ...
    def infer(self, model, case, rec):
        ...
        instrumentation, and `rb review verify-hook` proves it fired exactly {iters} times.
...
def integration_md(brief: BriefV1, adapter_id: Optional[str], metric: Metric, cfg_path: str) -> str:
    ...
    head = (
        ...
        f"Generated by `rb review onboard` from `{BRIEF_NAME}`. rb review onboard sent nothing; these are ordinary files in this directory.\n\n"
    ...
    if built_in:
        ...
        steps = [
            ...
            ("2", "Check the environment", "`rb review doctor --checkpoint " + cur + "`", "Config, device, model code, dataset, and each checkpoint's sha256, architecture and parameter count. Exit 3 and a named fix if something is wrong."),
            ("3", "Prove the instrumentation", f"`rb review verify-hook --checkpoint {cur}`", f"Runs one case and asserts the recorder fired exactly {iters} times. `E_HOOK_NOT_REACHABLE` or `E_HOOK_LENGTH` if not."),
            ("4", "Prove the numbers", f"`rb review verify-adapter --checkpoint {cur}`", "Compares the adapter's per-case error with the model repository's own evaluation path on a few cases. This is the step that makes the rest trustworthy; see below."),
            ("5", "A small run first", f"`rb review run --baseline {cur} --candidate {cand} --limit 5`", "Five cases end to end. Read `rb-runs/<run>/report.md`."),
            ("6", "The real review", f"`rb review run --baseline {cur} --candidate {cand}`", "Then `rb review findings <run> --top 5`, and `rb review case <run> <id> --render` for the case it names."),
        ]
    else:
        body = (
        ...
        steps = [
            ...
            ("3", "Loading and listing", "`rb review doctor`", "Fill TODO 1 (`load`) and TODO 2 (`cases`), then run this. It names what is still missing. Repeat until it passes."),
            ("4", "One case, instrumented", f"`rb review verify-hook --checkpoint {cur}`", f"Fill TODO 3 (`infer`) and TODO 4 (`metric_value`). This runs one case and asserts the recorder fired exactly {iters} times. The recorder is a forward hook; your model code is not edited."),
            ("5", "Prove the numbers", f"`rb review verify-adapter --checkpoint {cur}`", "Fill TODO 5 (`reference_value`) if you have an evaluation script. See below; this is the most valuable hour in the integration."),
            ("6", "A small run first", f"`rb review run --baseline {cur} --candidate {cand} --limit 5`", "Five cases end to end. Read `rb-runs/<run>/report.md`."),
            ("7", "The real review", f"`rb review run --baseline {cur} --candidate {cand}`", "Then `rb review findings <run> --top 5`, and `rb review case <run> <id> --render` for the case it names."),
        ]
    ...
    back = (
        ...
        "rb uploads nothing unless you run `rb workspace push`. If it worked and you are willing, `rb review share <run>` writes "
