...
unknown required knob keeps a claim from reading as established, a frozen spec changes only with a recorded reason,
...
from rabbit_brain.investigation import Knob, Source
...
def test_a_hand_edited_verified_knob_is_refused(lab, capsys):
    ...
    doc["knobs"] = [{"name": "lr", "value": 0.0001, "status": "verified", "required": True, "source": {"kind": "file", "path": "configs/train.yaml", "line": 2},
                     "note": "", "set_by": "agent:x", "set_at": "2026-09-24T00:00:00+00:00"}]
    p.write_text(json.dumps(doc))
    code, env = rbj(capsys, "show", "int8")
    assert code == 2 and env.errors[0].code == "E_LEDGER_CORRUPT" and any("Only `rb knob verify` sets verified" in pr for pr in env.errors[0].model_dump()["problems"])


# ---------------------------------------------------------------- knobs: only the tool verifies


def test_a_proposed_value_is_inferred_and_verify_reads_the_line(lab, capsys):
    experiment(capsys)
    code, env = rbj(capsys, "knob", "set", "int8", "lr", "0.0001", "--source", "configs/train.yaml:2")
    assert code == 0 and env.data["knob"]["status"] == "inferred" and env.next == ["rb knob verify int8 lr"]
    code, env = rbj(capsys, "knob", "verify", "int8", "lr")
    row = env.data["results"][0]
    assert code == 0 and row["ok"] and row["after"] == "verified" and row["read"] == "lr: 1e-4" and row["line"] == 2
    knob = Ledger(lab).load("experiment", "int8").knob("lr")
    assert knob.status == "verified" and knob.source.resolved.sha256
    if HAS_GIT:
        assert len(knob.source.resolved.commit) == 40   # a tracked, unmodified file is pinned to HEAD


def test_a_source_that_does_not_state_the_value_leaves_it_inferred(lab, capsys):
    experiment(capsys)
    code, env = rbj(capsys, "knob", "set", "int8", "batch_size", "8", "--source", "configs/train.yaml:3", "--verify")
    row = env.data["verify"][0]
    assert code == 1 and not row["ok"] and row["code"] == "E_SOURCE_UNRESOLVED" and "reads: batch_size: 6" in row["reason"]
    assert env.data["knob"]["status"] == "inferred"


def test_quote_search_and_optional_commit(lab, capsys):
    experiment(capsys)
    code, env = rbj(capsys, "knob", "set", "int8", "small", "false", "--source", "configs/train.yaml", "--quote", "small: false", "--verify")
    assert code == 0 and env.data["knob"]["status"] == "verified" and env.data["verify"][0]["line"] == 4
    code, env = rbj(capsys, "knob", "set", "int8", "model", "raft", "--source", "configs/train.yaml", "--verify")
    ...
    if HAS_GIT:
        ...
        code, env = rbj(capsys, "knob", "set", "int8", "lr", "1e-4", "--source", "configs/train.yaml:2", "--commit", head[:10], "--verify")
        assert code == 0 and env.data["knob"]["status"] == "verified" and env.data["knob"]["source"]["resolved"]["commit"] == head


def test_url_and_note_sources_are_recorded_but_cannot_verify(lab, capsys):
    experiment(capsys)
    code, env = rbj(capsys, "knob", "set", "int8", "cuda", "12.4", "--source", "https://example.com/env", "--verify")
    assert code == 1 and env.data["knob"]["status"] == "inferred" and env.data["verify"][0]["code"] == "E_SOURCE_UNVERIFIABLE"
    code, env = rbj(capsys, "knob", "set", "int8", "gpu", "A5000", "--source", "note:the authors said so in an issue")
    assert code == 0 and env.data["knob"]["source"]["kind"] == "note"
...
def test_a_run_source_reads_a_json_pointer(lab, capsys):
    ...
    code, env = rbj(capsys, "knob", "set", "int8", "seed", "1234", "--source", "run:run1#/seeds/torch", "--verify")
    assert code == 0 and env.data["knob"]["status"] == "verified"
    code, env = rbj(capsys, "knob", "set", "int8", "cuda", '"12.4"', "--source", "run:run1/record.json#/environment/cuda", "--verify")
    assert code == 0 and env.data["knob"]["value"] == "12.4" and env.data["knob"]["status"] == "verified"
    code, env = rbj(capsys, "knob", "set", "int8", "seed", "99", "--source", "run:run1#/seeds/torch", "--verify")
...
def test_changing_a_value_or_its_file_ends_verified(lab, capsys):
    experiment(capsys)
    rbj(capsys, "knob", "set", "int8", "lr", "1e-4", "--source", "configs/train.yaml:2", "--verify")
    code, env = rbj(capsys, "knob", "set", "int8", "lr", "1e-4", "--note", "same value, same source")
    assert env.data["knob"]["status"] == "verified"          # nothing about the value changed
    code, env = rbj(capsys, "knob", "set", "int8", "lr", "3e-4")
    assert env.data["knob"]["status"] == "inferred" and env.data["knob"]["source"] is None   # the old source stated the old value
    rbj(capsys, "knob", "set", "int8", "lr", "1e-4", "--source", "configs/train.yaml:2", "--verify")
    Path("configs/train.yaml").write_text("model: raft\nlr: 1e-4 \nbatch_size: 6\nsmall: false\n", encoding="utf-8")
    code, env = rbj(capsys, "status")
    assert env.data["experiments"][0]["stale"] == ["lr"] and env.data["gate"]["stale"]
    assert any("changed since it was verified" in o["what"] or "source has changed" in o["what"] for o in env.data["open"])
    Path("configs/train.yaml").write_text("lr: 3e-4\n", encoding="utf-8")
    code, env = rbj(capsys, "knob", "verify", "int8")
    assert code == 1 and env.data["results"][0]["after"] == "inferred"   # re-verification that fails demotes it
...
def test_value_parsing_and_numeric_matching():
    ...
    with pytest.raises(ValueError):
        Knob(name="lr", value=None, status="inferred", set_by="human:x", set_at="t")
    with pytest.raises(ValueError):
...
def test_an_unknown_required_knob_keeps_a_passing_claim_unestablished(lab, capsys):
    claim_setup(capsys)
    rbj(capsys, "knob", "set", "int8", "seed", "--unknown")
    ...
    rbj(capsys, "knob", "set", "int8", "seed", "--unknown", "--optional")
...
def test_evidence_from_an_rb_run_and_from_a_json_file(lab, capsys):
    ...
    rbj(capsys, "claim", "add", "No regressions", "--experiment", "int8", "--metric", "regressions", "--at-most", "0", "--amend", "unused")
    code, env = rbj(capsys, "show", "c2")
    assert env.data["verdict"]["status"] == "refuted" and any("built-in example data" in c for c in env.data["verdict"]["conditions"])
...
def test_freeze_blocks_spec_changes_until_amended(lab, capsys):
    claim_setup(capsys)
    rbj(capsys, "knob", "set", "int8", "lr", "1e-4", "--source", "configs/train.yaml:2")
    rbj(capsys, "evidence", "attach", "int8", "--metric", "delta_epe=0.01")
    ...
    code, env = rbj(capsys, "knob", "set", "int8", "lr", "3e-4", "--amend", "the config was updated before any held-out run")
...
def test_agents_propose_and_people_decide(lab, capsys, monkeypatch):
    ...
    code, env = rbj(capsys, "knob", "set", "int8", "seed", "--unknown")
    assert code == 0 and env.data["knob"]["set_by"] == "agent:claude-code"
...
def test_status_and_context_are_the_handoff(lab, capsys):
    ...
    rbj(capsys, "knob", "set", "int8", "seed", "--unknown")
    rbj(capsys, "knob", "set", "int8", "cuda", "12.4", "--source", "https://example.com/env")
    ...
    assert "open question: Can INT8 ship?" in whats and "int8: seed is unknown and required" in whats
    assert any("cuda = 12.4 is inferred; find a file or run" in w for w in whats) and any("c1 is not tested" in w for w in whats)
    ...
    assert "Only `rb knob verify` makes a knob verified" in md and "RB_ACTOR=agent:" in md
...
def test_the_documented_walkthrough_runs(lab, capsys):
    ...
    expected_exit = {"knob verify": 1, "status": 1}   # the url-sourced knob cannot verify; seed is unknown, so the gate fails
    ran = 0
    for line in block.splitlines():
        ...
        code = main(argv)
        capsys.readouterr()
        key = " ".join(argv[:2]) if argv[0] in ("knob",) else argv[0]
